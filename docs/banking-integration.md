...
- Bank balances are stored separately from recorded balances. Automatic opening-balance calibration is disabled: a live available balance and imported history are not a guaranteed matching snapshot. Set the opening balance explicitly from a dated statement in Accounts. The legacy reconcile endpoint returns a useful 409 rather than rewriting money.
...
## Product-overhaul diagnostics

Run `node --env-file=.env --import tsx scripts/banking-doctor.ts` (or select your sandbox env file) for presence/validity checks that never print secret values. `/api/auth/me` now distinguishes `restricted`, `disabled`, `unconfigured`, and `ready`. Ready means a provider is configured, not that remote authorization has been verified.

Callback state is claimed before the provider exchange, so concurrent replay cannot exchange twice. Duplicate identities across connections are rejected; use Reconnect on the original connection, including after disconnect. Provider currency changes are rejected rather than reinterpreting history. Recognized refunds inherit their original expense category.

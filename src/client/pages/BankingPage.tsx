...
type Connection = { syncing:boolean; id:string; provider:string; environment:string; institutionName:string; institutionCountry:string; status:string; consentExpiresAt:string|null; lastSyncedAt:string|null; nextSyncAt:string|null; errorCode:string|null; accounts:Link[] };
...
export default function BankingPage() {
  ...
  const me=useMe();
  const production=me.data?.bankingEnvironment==='production';
...
function BankLink({item,accounts,link,reconcile,canLink,production}:{item:Link;accounts:Account[];link:any;reconcile:any;canLink:boolean;production:boolean}) {
  ...
  return <div className="rounded-2xl border border-brand/10 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-semibold">{item.name} · {item.currency}</p><p className="text-xs text-muted">Last sync: {date(item.lastSyncedAt,timezone)}</p></div><Badge tone={item.accountId?'positive':'warning'}>{item.accountId?'Linked':'Needs linking'}</Badge></div>
    {item.reportedBalance!==null&&<p className="mt-2 text-sm">Bank balance: <strong>{formatMoney(item.reportedBalance,item.currency)}</strong>{item.ledgerBalance!==null&&<> · Recorded balance: <strong>{formatMoney(item.ledgerBalance,item.currency)}</strong></>}</p>}
    {!item.accountId&&canLink?<div className="mt-3 flex flex-wrap gap-2"><Button disabled={link.isPending} onClick={()=>link.mutate({id:item.id,accountId:null})}>Add as a new account</Button><Select aria-label="Link to existing money account" value={selected} onChange={(e)=>setSelected(e.target.value)}><option value="">Already tracking this account?</option>{accounts.filter((a)=>a.currency===item.currency&&!a.isArchived).map((a)=><option key={a.id} value={a.id}>{a.name}</option>)}</Select><Button variant="secondary" disabled={!selected||link.isPending} onClick={()=>link.mutate({id:item.id,accountId:selected})}>Link existing</Button><p className="w-full text-xs text-muted">{production?'The first imported history requires review before future automatic posting.':'Existing accounts require historical reconciliation before future imports can post automatically.'}</p></div>:item.accountId&&(item.reviewCount>0||((production||item.linkMode==='existing')&&!item.automaticPostAfter))?<RouterLink className="mt-3 inline-flex text-sm font-semibold text-brand underline" to="/banking/reconciliation">Review bank items{item.reviewCount>0?` (${item.reviewCount})`:''}</RouterLink>:null}
    {item.accountId&&item.balanceStatus!=='reconciled'&&<p className="mt-3 text-xs text-muted">Bank and recorded balances may differ while history is being reviewed. Set your opening balance from a dated bank statement in Accounts; pending payments can affect the bank balance.</p>}

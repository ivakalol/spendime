...
export function Field({ label, hint, error, children }: { label: string; hint?: string; error?: string; children: ReactNode }) { const {t}=useLanguage();return <label className="grid gap-2 text-sm font-semibold leading-none text-ink"><span>{t(label)}</span>{children}{hint && <span className="text-xs font-normal leading-relaxed text-muted">{t(hint)}</span>}{error && <span className="flex items-center gap-1.5 text-xs font-medium leading-relaxed text-rose-700"><AlertCircle className="size-3.5 shrink-0" />{t(error)}</span>}</label>; }
    ...
    <section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined} className="modal-panel animate-sheet" data-modal-panel>
      <div className="modal-header">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><h2 id={titleId} className="text-xl font-bold tracking-tight text-ink">{t(title)}</h2>{description && <p id={descriptionId} className="mt-1 max-w-md text-sm leading-relaxed text-muted">{t(description)}</p>}</div>
